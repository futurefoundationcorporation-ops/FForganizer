...
export default async function handler(req, res) {
  ...
  try {
    const { key } = req.body
    if (!key) return res.status(400).json({ ok: false, error: 'Chave não fornecida' })

    const masterKey = process.env.MASTER_KEY
    if (!masterKey) return res.status(500).json({ ok: false, error: 'MASTER_KEY não configurada' })
    ...
    if (key.trim() === masterKey.trim()) {
      ...
      const { data, error } = await supabase.rpc('validate_master_or_access_key', {
        input_key: key
      })
